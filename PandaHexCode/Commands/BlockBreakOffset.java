...
public class BlockBreakOffset extends ChatCommand{
	...
	public boolean onCommand(CommandSender arg0, Command arg1, String arg2, String[] arg3) {
		...
		if(arg3.length < 4)
			p.sendMessage(PluginMain.PR+"§cPlease use /blockbreakoffset <Player> <X> <Y> <Z>");
		else {
			if(Bukkit.getPlayer(arg3[0]) == null)
				...
				if(x == 0 && y == 0 && z == 0) {
					if(targetPlayers.contains(GetTargetPlayerWithName(arg3[0]))) {
						...
						p.sendMessage(PluginMain.PR+"§e"+arg3[0]+"§b can now break Blocks normal again!");
					}else
				...
				if(targetPlayers.contains(GetTargetPlayerWithName(arg3[0]))) {
					...
					p.sendMessage(PluginMain.PR+"§e"+arg3[0]+"§2 Changed offset (If you want standart use 0 0 0)!");
				}
				else {
					...
					p.sendMessage(PluginMain.PR+"§e"+arg3[0]+"§2 can now only break Blocks with offset haha!");
				}
			}
		}
		return false;
